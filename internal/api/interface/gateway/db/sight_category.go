...
func (r *SightCategoryRepository) FindOneByID(ctx context.Context, sightCategoryID uint64) (*model.SightCategory, error) {
	value := &model.SightCategory{}

	stmt, err := r.client.Preparex("SELECT * FROM sight_categories WHERE id = $1")
	if err != nil {
		return nil, fmt.Errorf("SightCategoryRepository FindOneByID: %w", err)
	}
	if err = stmt.GetContext(ctx, value, sightCategoryID); err != nil {
		return nil, fmt.Errorf("SightCategoryRepository FindOneByID: %w", err)
	}
...
func (r *SightCategoryRepository) FindByName(ctx context.Context, name string, offset, limit uint64) ([]*model.SightCategory, error) {
	...
	qb := sq.Select("*").From(model.SightCategory{}.TableName()).PlaceholderFormat(sq.Dollar)
